...
def main() -> None:
    ...
    parameter_file_extensions = {
        suffix.lstrip(".") for suffix in Parameters.PARAMETER_FILE_SUFFIXES
    }
    ...
    parser.add_argument(
        ...
        help="A JSON file containing parameters for the object being made. May be given more than once, in which case later files override earlier ones",
        default=None,
        action=CheckExt(parameter_file_extensions, append=True),
    )
    parser.add_argument(
        "-s",
        "--section",
    ...
    parser.add_argument(
        ...
        help="Add the switch type name and stabilizer type name to the filename",
        default=False,
        action="store_true",
    )
    parser.add_argument(
    ...
    if args.all_sections:
        ...
        for section in range(keyboard.get_top_section_count()):
            ...
            solid_object_dict[section]["top"] = color(section_color)(
                render()(keyboard.get_assembly(top=True))
            )
            solid_object_dict[section]["all"] = color(section_color)(
                render()(keyboard.get_assembly(all=True))
            )
            solid_object_dict[section]["plate"] = color(section_color)(
                render()(keyboard.get_assembly(plate_only=True))
            )

            # If there is a bottom section for the current section add it to section dict
            if section < keyboard.get_bottom_section_count():
                solid_object_dict[section]["bottom"] = color(section_color)(
                    render()(keyboard.get_assembly(bottom=True))
                )
                solid_object_dict[section]["case_bottom"] = color(section_color)(
                    render()(keyboard.get_assembly(case_bottom=True))
                )
    ...
    # Create exploded object
        ...
        for section in range(keyboard.get_top_section_count()):
            keyboard.set_section(section)
            section_color = SECTION_COLORS[section % len(SECTION_COLORS)]
            solid_object_dict[-1]["top"] += color(section_color)(
                up(5 * section)(
                    right(10 * section)(render()(keyboard.get_assembly(top=True)))
                )
            )
            solid_object_dict[-1]["plate"] += color(section_color)(
                up(5 * section)(
                    right(10 * section)(
                        render()(keyboard.get_assembly(plate_only=True))
                    )
            ...
            if section < keyboard.get_bottom_section_count():
                solid_object_dict[-1]["bottom"] += color(section_color)(
                    up(5 * section)(
                        right(10 * section)(
                            render()(keyboard.get_assembly(bottom=True))
                        )
                ...
                solid_object_dict[-1]["case_bottom"] += color(section_color)(
                    up(5 * section)(
                        right(10 * section)(
                            render()(keyboard.get_assembly(case_bottom=True))
                        )
    ...
    # Create objects for a specified section
        ...
        solid_object_dict[args.section]["top"] = color(section_color)(
            render()(keyboard.get_assembly(top=True))
        )
        solid_object_dict[args.section]["all"] = color(section_color)(
            render()(keyboard.get_assembly(all=True))
        )
        solid_object_dict[args.section]["plate"] = color(section_color)(
            render()(keyboard.get_assembly(plate_only=True))
        )

        # If there is a bottom section for the current section add it to section dict
        if args.section < keyboard.get_bottom_section_count():
            solid_object_dict[args.section]["bottom"] = color(section_color)(
                render()(keyboard.get_assembly(bottom=True))
            )
            solid_object_dict[args.section]["case_bottom"] = color(section_color)(
                render()(keyboard.get_assembly(case_bottom=True))
            )
    ...
    # Create an objects that are not split into sections. No other options were specified
        ...
        solid_object_dict["all"]["bottom"] = render()(
            keyboard.get_assembly(bottom=True)
        )
        solid_object_dict["all"]["all"] = render()(keyboard.get_assembly(all=True))
        solid_object_dict["all"]["plate"] = render()(
            keyboard.get_assembly(plate_only=True)
        )
        solid_object_dict["all"]["case_bottom"] = render()(
            keyboard.get_assembly(case_bottom=True)
        )
    ...
    print(parameters)
    print(
        f"Case Height: {parameters.real_case_height:f}, Case Width: {parameters.real_case_width:f}\n"
    )
    ...
    for section in solid_object_dict:
        ...
        for part_name in solid_object_dict[section]:
            ...
            if solid_object_dict[section][part_name] is not None:
                ...
                if isinstance(section, int) and section > -1:
                    assembly_includes.setdefault(part_name, []).append(
                        scad_file_name.name
                    )
    ...
    if args.all_sections:
        assembly_switch_postfix = (
            ("_" + parameters.switch_type) if args.switch_type_in_filename else ""
        )
        assembly_stab_postfix = (
            ("_" + parameters.stabilizer_type) if args.switch_type_in_filename else ""
        )
        for part_name, section_files in assembly_includes.items():
            assembly_file_name = scad_folder_path / (
                layout_name
                + "_assembled_"
                + part_name
                + assembly_switch_postfix
    ...
    print(
        "  %d top section(s), %d bottom section(s)"
    ...
    for section_number, (width, height) in enumerate(section_dimensions):
        ...
        print(
            "    Section %d: %.1f x %.1f mm%s"
            % (
                section_number,
                width,
                height,
                "" if fits else "  (exceeds build plate!)",
            )
        )
