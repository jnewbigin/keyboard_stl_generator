...
def example_files():
    root_files = [
        path
        for path in sorted(REPO_ROOT.glob("parameters*.json"))
        if path != SCHEMA_PATH
    ]
    test_files = sorted((REPO_ROOT / "parameter_test_files").glob("*.json"))
    return [
        path for path in root_files + test_files if path.name not in INVALID_ON_PURPOSE
    ]
...
class TestSchemaCoverage:
    ...
    def test_no_property_describes_a_parameter_that_does_not_exist(self, schema):
        ...
        assert (
            described - known == set()
        ), "parameters.schema.json describes unknown parameters"
...
class TestExampleFiles:
    ...
    def test_example_file_validates(self, schema, file_path):
        jsonschema.validate(
            json5.loads(Parameters.read_parameter_file(file_path)), schema
        )
...
class TestRejectsMistakes:
    ...
    def test_circle_needs_a_radius_or_diameter(self, schema):
        with pytest.raises(jsonschema.ValidationError):
            jsonschema.validate(
                {"custom_polygons": [{"type": "circle", "coordinates": [[0, 0]]}]},
                schema,
            )
    ...
    def test_polygon_needs_points(self, schema):
        with pytest.raises(jsonschema.ValidationError):
            jsonschema.validate(
                {"custom_polygons": [{"type": "polygon", "coordinates": [[0, 0]]}]},
                schema,
            )
