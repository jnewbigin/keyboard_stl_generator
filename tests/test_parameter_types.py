...
class TestBooleanParameters:
    ...
    def test_quoted_false_is_rejected(self):
        with pytest.raises(
            ValueError, match="cable_hole: 'false' is not of type 'boolean'"
        ):
    ...
    def test_nullable_boolean_rejects_a_string(self):
        with pytest.raises(
            ValueError, match="custom_pcb: 'false' is not of type 'boolean', 'null'"
        ):
...
class TestNonFiniteNumbers:
    ...
    def test_infinity_is_rejected(self):
        with pytest.raises(
            ValueError, match="x_build_size: inf is not a finite number"
        ):
