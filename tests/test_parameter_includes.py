...
class TestSingleFile:
    ...
    def test_schema_key_is_not_a_parameter(self, tmp_path):
        file_path = write(tmp_path, 'parameters.json',
...
class TestIncludeOrder:
    def test_includes_are_merged_in_list_order(self, tmp_path):
        ...
        file_path = write(tmp_path, 'parameters.json', {'include': ['first.json', 'second.json']})

        assert Parameters.load_parameter_files([file_path]) == {'case_height': 20, 'kerf': 0.1}

    def test_reversing_the_list_reverses_the_winner(self, tmp_path):
        write(tmp_path, 'first.json', {'case_height': 10})
        write(tmp_path, 'second.json', {'case_height': 20})
        file_path = write(tmp_path, 'parameters.json', {'include': ['second.json', 'first.json']})
    ...
    def test_own_keys_override_includes(self, tmp_path):
        ...
        file_path = write(tmp_path, 'parameters.json',
                          {'include': ['base.json'], 'case_height': 18})

        assert Parameters.load_parameter_files([file_path]) == {'case_height': 18, 'kerf': 0.1}
    ...
    def test_own_keys_win_even_when_written_above_the_include(self, tmp_path):
        ...
        write(tmp_path, 'base.json', {'case_height': 10})
        file_path = tmp_path / 'parameters.json'
        file_path.write_text('{"case_height": 18, "include": ["base.json"]}', encoding='utf-8')
    ...
    def test_nested_values_are_replaced_not_merged(self, tmp_path):
        write(tmp_path, 'base.json', {'custom_switch': {'points': [[7, 7]], 'path': [0]}})
        file_path = write(tmp_path, 'parameters.json',
                          {'include': ['base.json'], 'custom_switch': {'points': [[8, 8]]}})

        assert Parameters.load_parameter_files([file_path]) == {'custom_switch': {'points': [[8, 8]]}}

    def test_include_must_be_a_string_or_list_of_strings(self, tmp_path):
        file_path = write(tmp_path, 'parameters.json', {'include': {'file': 'base.json'}})
...
class TestRejectsBadArguments:
    ...
    def test_directory_is_reported_as_a_directory(self, tmp_path):
        ...
        with pytest.raises(IsADirectoryError, match='is a directory, not a parameter file'):
            Parameters.load_parameter_files([file_path])
...
class TestNestedIncludes:
    def test_includes_are_resolved_depth_first(self, tmp_path):
        ...
        write(tmp_path, 'base.json', {'include': ['printer.json'], 'kerf': 0.2, 'case_height': 10})
        file_path = write(tmp_path, 'parameters.json',
                          {'include': ['base.json'], 'case_height': 18})

        assert Parameters.load_parameter_files([file_path]) == {
            'x_build_size': 150, 'kerf': 0.2, 'case_height': 18,
        }
    ...
    def test_include_paths_are_relative_to_the_including_file(self, tmp_path):
        ...
        write(tmp_path, 'shared/base.json', {'include': ['printer.json'], 'case_height': 10})
        file_path = write(tmp_path, 'boards/parameters.json',
                          {'include': ['../shared/base.json']})

        assert Parameters.load_parameter_files([file_path]) == {
            'x_build_size': 150, 'case_height': 10,
        }

    def test_absolute_include_path_is_used_as_given(self, tmp_path):
        base_path = write(tmp_path, 'shared/base.json', {'case_height': 10})
        file_path = write(tmp_path, 'boards/parameters.json', {'include': [str(base_path)]})
    ...
    def test_diamond_include_is_allowed(self, tmp_path):
        ...
        file_path = write(tmp_path, 'parameters.json', {'include': ['left.json', 'right.json']})

        assert Parameters.load_parameter_files([file_path]) == {
            'x_build_size': 150, 'case_height': 10, 'kerf': 0.2,
        }
