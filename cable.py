...
class Cable:
    ...
    def __init__(self, parameters: Parameters) -> None:
        ...
        self.inner_block_width = (
            self.parameters.cable_hole_width
            - (self.inner_block_clip_offset * 2)
            - (self.clip_arm_thickness * 2)
        )
        ...
        self.cable_holder_height = (
            self.parameters.cable_hole_height - self.cable_hole_holder_offset
        )  # (self.parameters.kerf * 4)
        ...
        self.clamp_percentage = 0.9
        self.clamp_holder_gap = 0.1  # self.parameters.kerf * 2
        self.clamp_block_height = self.cable_holder_height / 4
        self.clamp_block_width = (
            self.inner_block_width * self.clamp_percentage
        )  # - (self.clamp_holder_gap * 2)
        # Set a value that will reduce the height of the cable hole when the clamp is inserted
        self.clamp_hole_offset = 0.1
        self.clamp_height_percent_of_holder_height = 0.75
    ...
    def holder_full(self) -> OpenSCADObject:
        ...
        cable_hole_plate_width = self.parameters.cable_hole_width + (
            self.stop_plate_overhang * 2
        )
        ...
        holder = cube(
            [
                cable_hole_plate_width,
                self.cable_holder_height,
                self.stop_plate_thickness,
            ],
            center=True,
        )
        ...
        clip_part = right(
            ...
            - self.clip_arm_thickness
            - (self.cable_hole_holder_offset / 2)
        )(clip_part)
        ...
        holder += up(
            (self.parameters.case_wall_thickness / 2) + (self.stop_plate_thickness / 2)
        )(
            cube(
                [
                    self.inner_block_width,
                    self.cable_holder_height,
                    self.parameters.case_wall_thickness,
                ],
                center=True,
            )
        )
    ...
    def clip(self) -> OpenSCADObject:
        ...
        poly_points = [
            ...
            [
                self.clip_arm_thickness + self.clip_short_edge,
                self.parameters.case_wall_thickness + self.clip_body_gap,
            ],
            [
                self.clip_arm_thickness,
                self.parameters.case_wall_thickness + self.clip_body_gap,
    ...
    def holder_clamp(self, clamp_remove_block: bool = False) -> OpenSCADObject:
        ...
        clamp_depth = self.parameters.case_wall_thickness + (
            self.stop_plate_thickness * 3
        )

        # Create clamp block
        clamp_block = forward(
            (self.cable_holder_height / 2)
            - (self.clamp_block_height / 2)
            + (clamp_holder_gap)
        )(
            cube(
                [
                    self.clamp_block_width + (clamp_holder_extra * 2),
                    self.clamp_block_height - (clamp_holder_gap * 2),
                    clamp_depth,
                ],
                center=True,
            )
        ...
        clamp_height = (
            self.cable_holder_height * self.clamp_height_percent_of_holder_height
        )

        # Set offset for surround block height
        surround_block_height_offset = (
            self.cable_holder_height - clamp_height
        )  # (clamp_holder_gap * 24)

        # Create surround block
        surround_block = forward(
            surround_block_height_offset / 2
        )(  # forward(self.clamp_block_height - self.clamp_hole_offset) (
            cube(
                [
                    self.clamp_block_width + (clamp_holder_extra * 2),
                    clamp_height,
        ...
        clamp_block += up(
            (clamp_depth / 2) + self.stop_plate_thickness + clamp_holder_gap
        )(surround_block)
        clamp_block += down(self.stop_plate_thickness + clamp_holder_gap)(
            surround_block
        )

        # Remove a 180 degree rotated version of the cable hole with a slot from the clamp block
        clamp_block -= back(self.clamp_hole_offset)(
            rotate(180, [0, 0, 1])(self.holder_hole(include_slot=True))
        )
    ...
    def get_cable_hole(self) -> OpenSCADObject:
        ...
        if self.parameters.cable_hole:
            ...
            return up(
                ...
                - (self.parameters.cable_hole_height / 2)
                - self.parameters.plate_thickness
                - self.parameters.cable_hole_down_offset
            )(
                right(self.parameters.cable_hole_center_x())(
                    forward(
                        ...
                        cube(
                            [
                                self.parameters.cable_hole_width,
                                self.parameters.case_wall_thickness * 2,
                                self.parameters.cable_hole_height,
                            ],
                            center=True,
                        )
                    )
